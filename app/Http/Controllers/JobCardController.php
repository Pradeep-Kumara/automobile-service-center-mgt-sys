...
class JobCardController extends Controller
{
    ...
    public function jobCardIndex(){
     ...
     $orderNo=Order::where('order_status',1)->get();   
     $items=Item::where('item_status',1)->get(); 
     $tasks=Task::where('task_status',1)->get();   
     $customers=Customer::where('customer_status',1)->get();

     return view('job-card',['title'=>'Job Card','tasks'=>$tasks,'items'=>$items,'customers'=>$customers,'orderNo'=>$orderNo]);
    }
    ...
    public function viewJobTableData(Request $request){
        ...
        if(count($orderItem)==0){
            ...
            foreach ($orderItem as $order) {
               ...
               $tableData .= "<td>";
               $tableData .= "<button type='button'  class='btn btn-sm btn-basic  waves-effect waves-light'  data-target=\"#viewItemModal\" data-toggle=\"modal\"   data-id='$order->job_item_temp_line_id'   id='viewItem' > ";
               $tableData .= " <i class=\"fa fa-eye\"></i>";
               $tableData .= "  </button>";
               $tableData .= "&nbsp;";
               $tableData .= "<button type='button'  class='btn btn-sm btn-basic  waves-effect waves-light'  onclick=deleteItem($order->job_item_temp_line_id) > ";
        ...
        foreach ($orderTasks as $order) {
           ...
           $tableData2 .= "<td>";
            ...
            $tableData2 .= "<button type='button'  class='btn btn-sm btn-basic  waves-effect waves-light'  data-target=\"#viewTaskModal\" data-toggle=\"modal\"   data-id='$order->job_task_temp_line_id'   id='viewTask' > ";
            $tableData2 .= " <i class=\"fa fa-eye\"></i>";
            $tableData2 .= "  </button>";
            $tableData2 .= "&nbsp;";
            $tableData2 .= "<button type='button'  class='btn btn-sm btn-basic  waves-effect waves-light'  onclick=deleteTask($order->job_task_temp_line_id) > ";
    ...
    public function loadTempData(Request $request){
        ...
        foreach( $orderTaskDetails as  $orderTaskDetail){
            ...
            $taskTotal+=$orderTaskDetail->order_task_price*$orderTaskDetail->order_task_duration;
        ...
        $refundAmount=$orderDetail->order_require_refund_amount;
        return response()->json(['refundAmount'=>$refundAmount,'orderDetail'=>$orderDetail,'vehicleNo'=>$vehicleNo,'mobileNo'=>$mobileNo,'itemTotal'=>$itemTotal,'total'=>$total]);
    ...
    public function store(Request $request){
        ...
        $orderID=$request['orderID'];
        //$jobStartTimeFmt = Carbon::parse('jobStartTime');
        //$jobEndTimeFmt = Carbon::parse('jobEndTime');
        //$jobDuration= $jobEndTimeFmt->diffInSeconds($jobStartTimeFmt);
        //$jobDuration= strtotime($jobEndTime) - strtotime($jobStartTime);

        $jobStartTimeFmt = new DateTime($jobStartTime);
        $jobEndTimeFmt = new DateTime($jobEndTime);
        $difference = $jobStartTimeFmt ->diff($jobEndTimeFmt );
        $jobDuration = $difference ->format('%H:%I:%S');
        ...
        $saveJobCard->jobcard_duration=$jobDuration;
        $saveJobCard->jobcard_status=1;
        $saveJobCard->jobcard_total= $taskTotal+ $itemTotal+$orderDetail->order_require_refund_amount;
        $saveJobCard->jobcard_advance_paid=$orderDetail->order_advance_payment;
        $saveJobCard->jobcard_paid_amount=0;
        $saveJobCard->jobcard_user_id=Auth::user()->user_id;
        $saveJobCard->order_order_id=$orderID;
        $saveJobCard->jobcard_customer_id=$orderDetail->order_customer_id;
        $saveJobCard->save();
        ...
        foreach($orderTaskDetails as  $orderTaskDetail){
            ...
            $taskPurCost=Task::find($orderTaskDetail->job_task_task_id);
                ...
                $saveItem->job_task_purchase_cost=$taskPurCost->task_cost;
                $saveItem->save();
        ...
        return response()->json(['success'=>'Job is ready for payment']);
    }
